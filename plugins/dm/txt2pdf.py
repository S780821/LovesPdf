...
@LovesPdf.on_message(filters.private & filters.command(["txt2pdf"]) & ~filters.edited & filters.incoming)
...
@LovesPdf.on_callback_query(txt2pdf)
...
@LovesPdf.on_callback_query(txt2pdfBack)
...
@LovesPdf.on_callback_query(pgSize)
async def _pgSize(bot, callbackQuery):
    try:
        ...
        PROCESS.append(chat_id); TXT[chat_id] = []; xmartperson=True
        while(xmartperson):
            ...
            if askPDF.text == "/exit":
                ...
                TXT[chat_id].append(None); xmartperson = False
            elif askPDF.text:
                TXT[chat_id].append(f"{askPDF.text}"); xmartperson = False
        # xmartperson=True ONLY IF PROCESS CANCELLED
        if xmartperson == True:
